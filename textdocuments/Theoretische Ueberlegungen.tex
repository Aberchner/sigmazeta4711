...
\begin{document}
	\section{Martingale und Varianten}
		...
		\subsection{Verallgemeinertes Martingale}
			...
			\[\sum_{i=1}^{n}((\gamma +1)p_{i}-1)^2=4\omega^2\lambda^2\]
			\[\lambda = \pm\frac{\sqrt{\sum_{i=1}^{n}((\gamma +1)p_{i}-1)^2}}{2\omega}\]
			Eingesetzt in die anderen n Gleichungen folgt
			\[x_{i}=\mp\frac{2\omega((\gamma +1)p_{i}-1)}{\sqrt{\sum_{j=1}^{n}((\gamma +1)p_{j}-1)^2}}\]\\
			...
			$$
			x_{i}=\frac{2\omega((\gamma +1)p_{i}-1)}{\sqrt{\sum_{j=1}^{n}((\gamma +1)p_{j}-1)^2}}
			$$
\end{document}